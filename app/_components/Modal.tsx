...
export default function Modal({ data }: { data: Thought }) {
	const { title, thought } = data

	return (
		<section className="bg-primary-500/20 fixed top-0 left-0 z-50 h-screen w-full backdrop-blur-xs transition duration-500">
			<div className="border-primary-200/80 fixed top-1/2 left-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-3 rounded-lg border bg-white p-6 shadow-xs">
